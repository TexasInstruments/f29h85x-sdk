...
{
    if(inst.USE_PORT_OPTIMISED_TASK_SELECTION == true && inst.MAX_PRIORITIES > 32)
    {
        validation.logError(
            "Maximum of 32 priorities are allowed with port optimised task selection",inst,"MAX_PRIORITIES");
    }

    if(inst.USE_MINI_LIST_ITEM == true)
    {
        validation.logError(
            "configUSE_MINI_LIST_ITEM must be set to 0 for correct functioning of C29x port",inst,"USE_MINI_LIST_ITEM");
    }
    ...
    if(inst.APPLICATION_ALLOCATED_HEAP == true && inst.SUPPORT_DYNAMIC_ALLOCATION == false)
    {
        validation.logWarning(
            "Dynamic allocation must be enabled to take effect",inst,"APPLICATION_ALLOCATED_HEAP");
    }
...
var freeRTOSModule = {
    ...
    templates: {
        ...
        "/driverlib/rov/syscfg_c.rov.xs.xdt": ""
    },
    config              : config,
    validate            : onValidate,
};
