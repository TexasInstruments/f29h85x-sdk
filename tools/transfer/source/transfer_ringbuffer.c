...
void TransferRingBuffer_init(TransferRingBuffer_Handle trb, char** buffer, uint16_t size, uint16_t numElemPerBuffEntry)
{
    trb->buffer = buffer;
    trb->bufferSize = size;
    trb->writeIndex = 0;
...
//
uint16_t TransferRingBuffer_read(TransferRingBuffer_Handle trb, char* data)
{
    uint16_t r = 0;
    uint16_t elemIndex = 0;
    char* bufferEntry = &(
            ((char*)trb->buffer)[trb->numElemPerBuffEntry*trb->readIndex]
                              );
...
//
uint16_t TransferRingBuffer_write(TransferRingBuffer_Handle trb, char* data)
{
    uint16_t r = 0;
    uint16_t elemIndex = 0;
    char* bufferEntry = &(
            ((char*)trb->buffer)[trb->numElemPerBuffEntry*trb->writeIndex]
                              );
...
// Returns 0 if fail (full)
//
uint16_t TransferRingBuffer_writeWithPad(TransferRingBuffer_Handle trb, char* data, uint16_t dataSize, uint16_t pad)
{
    uint16_t r = 0;
    uint16_t elemIndex = 0;
    char* bufferEntry = &(
...
// Returns 0 if fail (full)
//
uint16_t TransferRingBuffer_writePartialElement(TransferRingBuffer_Handle trb, uint16_t elemWriteStartIndex, char* data, uint16_t dataSize)
{
    uint16_t r = 0;
    uint16_t elemIndex = 0;
...
// Returns 0 if fail (full)
//
uint16_t TransferRingBuffer_writePartialElementWithConst(TransferRingBuffer_Handle trb, uint16_t elemWriteStartIndex, char data, uint16_t dataSize)
{
    uint16_t r = 0;
    uint16_t elemIndex = 0;
    char* bufferEntry = &(
