...
let device_driverlib_memmap = system.getScript("/driverlib/device_driverlib_peripherals/" + Cmn.getDeviceName().toLowerCase() + "_memmap.js");
...
[
    {
        ...
        getValue         : (inst, ui) =>
        {
            if(Common.isContextCPU1()) return 1;
            if(Common.isContextCPU2()) return 2;
            if(Common.isContextCPU3()) return 3;
    ...
    }, */
   {
    ...
    },
    {
        ...
        longDescription : `Use this field to place the ${currentContext} NMI Vector at a specific address (This will override any auto-assigned address if present). To auto-assign the vector address, set this field to 0x0.`,
        hidden          : Common.isContextCPU1(),
        default         : 0x0,
        displayFormat   : "hex"
    },
    {
        ...
        longDescription : `Use this field to place the ${currentContext} Reset Vector at a specific address (This will override any auto-assigned address if present). To auto-assign the vector address, set this field to 0x0.`,
        hidden          : Common.isContextCPU1(),
        default         : 0x0,
        displayFormat   : "hex"
    },
...
function config_static_main_core(core){
return  [
            {
                name    : 'r_aprFullMap',
                default : "",
                ...
                getValue: () =>
                {
                    ...
                    if(!aprList) return JSON.stringify(ret)
                    ret = aprList.map(function(item) {
                        delete item.memRegion;
                        return item;
                    });
            ...
            },
            {
                ...
                getValue: () =>
                {
                    let memoryList = Common.getMemoryList_configure(core)
                    return memoryList
                }
            },
            {
                ...
                getValue: () =>
                {
                    let flashSpace = Common.allocateAllMemoryRegions()["flashSpace"]
                    return JSON.stringify(flashSpace)
                }
            },
            {
                name    : 'r_cpu3FlashRegion',
                default : "",
                shouldBeAllocatedAsResource: true,
                hidden  : true,
                getValue: () =>
                {
                    let ret = Common.getFlashRegion(Common.CONTEXT_CPU3)
                    return JSON.stringify(ret)
                }
            },
            {
                ...
                getValue: (inst) =>
                {
                    let sysSecCurrCore = Common.modStaticByCPU(`/ti/security/System_Security`, core);
                    let sectionList = Common.getSectionsList_configure(sysSecCurrCore, core, inst)
                    return sectionList
                }
            },
            {
            ...
            },
            {
                name            : "nmiVectorAddress",
                displayName     : "NMI Vector Address",
                longDescription : `Use this field to place the NMI Vector at a specific address (This will override any auto-assigned address if present). To auto-assign the vector address, set this field to 0x0.`,
                hidden          : true,
                default         : 0x0,
                displayFormat   : "hex"
            },
            {
                name            : "resetVectorAddress",
                displayName     : "Reset Vector Address",
                longDescription : `Use this field to place the Reset Vector at a specific address (This will override any auto-assigned address if present). To auto-assign the vector address, set this field to 0x0.`,
                hidden          : true,
                default         : 0x0,
                displayFormat   : "hex"
            },
...
function config_static_non_main_core(core){
return  [
            {
                name    : 'r_aprFullMap',
                default : "",
...
[
    ...
    },
    {
        ...
        options     : (inst) =>
        {
            ...
            let sysSec = Common.modStaticByCPU(`/ti/security/${corePrefix}System_Security`, "CPU1");
            if(sysSec)
                return typeOptions;
    ...
    },
    {
        ...
        [
            ...
            },
            {
                ...
                getValue        : (inst, vo) =>
                {
                    ...
                    else
                    {
                        ...
                        let addedContext = inst.diffContext ? inst.diffContextName : reqdContext
            ...
            },
            {
                ...
                {
                    let reqdContext = check ? Common.getCurrentCorePrefix(inst).slice(0,-1) : currentContext;
                    return Common.getCoreList(1).map(x=> { return {name:x.name}}).filter(x => (x.name != reqdContext))
                },
                default         : [],
                hidden          : false,
                minSelections   : 0,
            },
            {
                ...
                description : "INTERNAL. If true, certain configs are hidden/locked",
                default     : false,
                hidden      : true
            },
        ]
    },
    {
        ...
        [
            ...
            },
            {
                ...
                {
                    if(inst.memType == "Flash")
                        return dmaMpu_instances.slice(0,1)
...
{
    // CHECK: Validate user-configured address ranges
    ...
    // CHECK: validate peripheral overlap
}
...
function validate(inst, vo)
{
    ...
    {
        // CHECK:  Peripherals selected are contiguous
        ...
        // CHECK: Same Peripheral is used in multiple APRs
        if(APRList)
        {
            APRList
    ...
    // CHECK: Flash DMA MPU instance validation
    ...
    // INFO: APR belonging to different context
    if((inst.diffContext) && (inst.diffContextName != reqdContext))
    ...
    if(sysSec)
    {
        if(sysSec.ssumode != 1)
        {
            ...
            let ext_apr = Common.modInstances(`/ti/security/${corePrefix}APR`, inst).filter(apr => apr.useExtAP)   //!! Move to sysec
            ...
            // Total # of APR check is done in System Security module
        }
    }
    else
    {
    ...
    // CHECK: Flash access availability
    if(inst.memType == "Flash" && reqdContext == Common.CONTEXT_CPU2 && inst.$name != "APR_FlashLoad_CPU2")
        vo.logError("Cannot create flash regions on current core", inst, 'memType')

    if(inst.memType == "Flash" && reqdContext == Common.CONTEXT_CPU3 && inst.$name != "APR_FlashLoad_CPU3" && bankMode < 2 )
        vo.logError("Cannot create flash regions on current core", inst, 'memType')
}
...
exports =
{
    ...
    config_static_main_core: config_static_main_core,
    config_static_non_main_core: config_static_non_main_core,
    validate: validate,
