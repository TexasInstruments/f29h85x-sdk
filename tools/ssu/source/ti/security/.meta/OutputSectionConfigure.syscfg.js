...
const currentContext    = Common.currentContext();
const raData            = system.resourceAllocation.data;
let   raDataCurrCtx     = raData.contexts.find(a => a["contextName"] == currentContext)
...
[
    ...
    },
    {
        ...
        options     : () => 
        {
            if(_.isEmpty(raDataCurrCtx))
                return []
            let aprsCurrCtx =   raDataCurrCtx
                                ["modules"]
                                .find(a => (a["moduleName"] == `/ti/security/${currentContext}_APR`))
            if(aprsCurrCtx)
            {
...
function validate(inst, vo)
{
    ...
    let aprsCurrCtx =   raData.contexts
                        .find(a => a["contextName"] == currentContext)
                        ["modules"]
                        .find(a => (a["moduleName"] == `/ti/security/${currentContext}_APR`))
    if(aprsCurrCtx)
    {
        let aprResources    = aprsCurrCtx.moduleStatic.requiredArgs
        aprMap              = JSON.parse(aprResources.r_aprFullMap)
    }
    ...
    if(runMem != "None")
    {
        let currRegion  = aprMap.find(apr => {return (apr.name == runMem)})
