#include <iostream>
#include <systemc.h>
#include "../type3/CLB_HLC_SC_model.h"
#include "../type3/hlc_c_model_oper.h"
...
TENOR_CLB_HLC::TENOR_CLB_HLC(sc_core::sc_module_name _nm,
			...
			sc_module(_nm)
 {
    ...
    SC_METHOD(spi_export);
    sensitive << P_EVENT_BUS_IN[0].pos() << P_EVENT_BUS_IN[1].pos() << P_EVENT_BUS_IN[2].pos() << P_EVENT_BUS_IN[3].pos() << P_EVENT_BUS_IN[4].pos() << P_EVENT_BUS_IN[5].pos() 
              << P_EVENT_BUS_IN[6].pos() << P_EVENT_BUS_IN[7].pos() << P_EVENT_BUS_IN[8].pos() << P_EVENT_BUS_IN[9].pos() << P_EVENT_BUS_IN[10].pos() << P_EVENT_BUS_IN[11].pos() 
              << P_EVENT_BUS_IN[12].pos() << P_EVENT_BUS_IN[13].pos() << P_EVENT_BUS_IN[14].pos() << P_EVENT_BUS_IN[15].pos() << P_EVENT_BUS_IN[16].pos() << P_EVENT_BUS_IN[17].pos() 
              << P_EVENT_BUS_IN[18].pos() << P_EVENT_BUS_IN[19].pos() << P_EVENT_BUS_IN[20].pos() << P_EVENT_BUS_IN[21].pos() << P_EVENT_BUS_IN[22].pos() << P_EVENT_BUS_IN[23].pos() 
              << P_EVENT_BUS_IN[24].pos() << P_EVENT_BUS_IN[25].pos() << P_EVENT_BUS_IN[26].pos() << P_EVENT_BUS_IN[27].pos() << P_EVENT_BUS_IN[28].pos() << P_EVENT_BUS_IN[29].pos() 
              << P_EVENT_BUS_IN[30].pos() << P_EVENT_BUS_IN[31].pos() << P_clk.pos();
...
void TENOR_CLB_HLC::spi_export(void)
{
  spi_event = P_spi_event_trig.read();
  spi_shift = P_spi_shift_val.read();
  spi_export_enable = SPI_RXBUFF_EN.read();
...
/*//This loop determines the trigger for each event
  for (i = 0; i<=3; i=i+1) {
  event_val[i] = (event_mux_alt[i] == 0) ? event_mux_sel[i] : event_mux_sel[i];
}*/

    for (i = 0; i<=3; i=i+1) {
        //active_event_in[i] = P_EVENT_BUS_IN[event_mux_sel[i]].read();

        //This loop determines which event is active
...
// hlc_instr_rd_val = INSTR_ARRAY[hlc_instr_rd_ptr];
   ...
   P_push_data_en.write(push_data_en);  //push_data_en, push_data_out, and pull_data_en are unnecessary now that there's a FIFO implementation (keep in case user wants to see)
