...
SC_MODULE(TENOR_CLB_HLC)
{
  ...
  sc_in< sc_uint<32> > e2;
  sc_in< sc_uint<32> > e3;
  sc_in< sc_uint<5> > P_spi_event_trig;
  sc_in< sc_uint<5> > P_spi_shift_val;
  sc_in<bool> ae0;
  sc_in<bool> ae1;
  sc_in<bool> ae2;
  sc_in<bool> ae3;
...
// These will be outputs
  ...
  sc_out< sc_uint<16> > P_spi_export;
...
////////////// Instance specific variables ///////////////////
    ...
    unsigned long spi_event;
    unsigned long spi_shift;
    unsigned long spi_export_data;
    bool spi_export_enable;
//        std::string HLC_file_path;

  // process definitions
  void hlc_logic();
  void hlc_mem_init(); // Call this function every time global reset (P_rst_n) is asserted (low).
  void spi_export();   // This function is used to export to SPI using HLC events
