...
let customWaveform   = system.getScript("/clb_syscfg/source/CustomWaveform.js");
...
function validate(inst, vo)
{
    ...
    for (let i = 0; i < 8; ++i) {
        ...
        if(inst['boundaryInput'+i] == 'tileOutput')
        {
            ...
            var matches = 0;
            let tileInstances = system.modules["/clb_syscfg/source/TILE"].$instances;
            for(let j = 0; j < system.modules["/clb_syscfg/source/TILE"].$instances.length; j++)
            {
            ...
            //console.log(system.modules["/clb_syscfg/source/TILE"].$instances[0].$name)
            //Common.printDebugObject(system.modules["/clb_syscfg/source/TILE"].$instances[0].$name)
        }
    }
}
