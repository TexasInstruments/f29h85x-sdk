...
int main(void) {
  ...
  DF13_runTest(df13_controller);

  uint32_t avgCount = totalCount / NUM_ELEMENTS;
  printf("DCL DF13 total cycles = %d\nDCL DF13 average cycles = %d\n",
         totalCount, avgCount);
}
...
int DF13_runTest(DCL_DF13 *ctrl_handle) {
  ...
  for (i = 0; i < NUM_ELEMENTS; i++) {
    ...
    //
    // Run the controller
    // Note: DCL_runDF13() does not have an internal clamp
    //       Use DCL_runDF13Clamp(ctrl_handle, ek, max, min)
    //       instead if an internal clamp is needed.
    //       DCL also provides external clamp DCL_runClamp(*data, max, min)
    //
    uk = DCL_runDF13(ctrl_handle, ek);
  ...
  printf("DF13 test produced %d error\n", errors);
