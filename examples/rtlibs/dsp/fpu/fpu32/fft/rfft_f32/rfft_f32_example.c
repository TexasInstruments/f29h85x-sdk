//#############################################################################
//
// FILE:   rfft_f32_example.c
//
// TITLE:  Real FFT, specify the implementation type (Optimized C 
// or Optimized Assembly) and the number of stages/size in 
// rfft_f32_example.h, build and run the example
...
int32_t main(void)
{
    ...
    printf("Unpacking input...\n");
    // "Unpack" the real input data prior to bit reversal
    ...
    printf("RFFT Pass_count = %d, Fail_count = %d \n", pass, fail);
    printf("RFFT Cycles = %d, Timer overhead = %d \n", differenceCounter, overheadCounter);
    ...
    printf("Completed %d-pt RFFT Pass_count = %d, Fail_count = %d \n", FFT_SIZE,pass, fail);
}
