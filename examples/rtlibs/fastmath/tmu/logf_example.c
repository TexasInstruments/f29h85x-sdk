...
float32_t test_output_logf_rts[INPUT_SIZE];
float32_t test_output_logf_tmu[INPUT_SIZE];
...
int32_t main(void)
{
    ...
    printf("logf computed with RTS library call...\n");
    ...
    {
        test_output_logf_rts[i] = logf(num_input[i]);
    }
    endCounter = CPUTimer_getTimerCount(CPUTIMER0_BASE);
    differenceCounter_rts = startCounter - endCounter - overheadCounter;
    CPUTimer_stopTimer(CPUTIMER0_BASE);
    
    printf("logf computed with TMU...\n");
    ...
    for(i=0;i<INPUT_SIZE;i++)
    {
        test_output_logf_tmu[i] = logf_opt(num_input[i]);
    }
    endCounter = CPUTimer_getTimerCount(CPUTIMER0_BASE);
    ...
    for(i=0;i<INPUT_SIZE;i++)
    {
        printf("Input=%.7f,RTS output=%.10f,Golden output=%.10f,RTS Error = %.10f\n",num_input[i],test_output_logf_rts[i],test_golden[i],(test_output_logf_rts[i]-test_golden[i]));
        if(fabsf(test_output_logf_rts[i]-test_golden[i])<tolerance)
        {
            pass++;
        } else {
            fail++;
        ...
        printf("Input=%.7f,TMU output=%.10f,Golden output=%.10f,TMU Error = %.10f\n",num_input[i],test_output_logf_tmu[i],test_golden[i],(test_output_logf_tmu[i]-test_golden[i]));
        if(fabsf(test_output_logf_tmu[i]-test_golden[i])<tolerance)
        {
            pass++;
        } else {
            fail++;
    ...
    printf("Completed logf test Pass_count = %d, Fail_count = %d \n", pass, fail);
    printf("logf with RTS Cycles = %d, Timer overhead = %d \n", differenceCounter_rts/INPUT_SIZE, overheadCounter);
    printf("logf with TMU Cycles = %d, Timer overhead = %d \n", differenceCounter_tmu/INPUT_SIZE, overheadCounter);
