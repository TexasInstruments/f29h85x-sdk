...
extern const float32_t test_golden_acosf;
...
int32_t main(void)
{
    ...
    // acosf
        ...
        test_output = acosf_opt(test_input);

        endCounter = CPUTimer_getTimerCount(CPUTIMER0_BASE);
        differenceCounter = startCounter - endCounter - overheadCounter;
        CPUTimer_stopTimer(CPUTIMER0_BASE);

        printf("acosf() computed with builtin/intrinsic TMU instructions...\n");
        printf("%.7f, %.7f,Error = %.7f\n",(test_output*TWOPI),test_golden_acosf,((test_output*TWOPI)-test_golden_acosf));
        if(fabsf((test_output*TWOPI)-test_golden_acosf)<tolerance)
        {
            pass++;
        } else {
            fail++;
        ...
        test_output = acosf(test_input);
        
        endCounter = CPUTimer_getTimerCount(CPUTIMER0_BASE);
        differenceCounter = startCounter - endCounter - overheadCounter;
        CPUTimer_stopTimer(CPUTIMER0_BASE);

        printf("acosf() computed with RTS library call...\n");
        printf("%.7f, %.7f,Error = %.7f\n",test_output,test_golden_acosf,(test_output-test_golden_acosf));
        if(fabsf(test_output-test_golden_acosf)<tolerance)
        {
            pass++;
        } else {
            fail++;
    ...
    printf("Completed atanf, acosf test Pass_count = %d, Fail_count = %d \n", pass, fail);
    printf("acosf Cycles = %d, Timer overhead = %d \n", differenceCounter, overheadCounter);
}
