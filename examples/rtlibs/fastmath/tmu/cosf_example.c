...
int32_t main(void)
{
    ...
    CPUTimer_stopTimer(CPUTIMER0_BASE);
        ...
        printf("cosf() computed with -ffast-math compiler option, resulting in TMU instruction...\n");
    ...
    #else
        printf("cosf() computed without -ffast-math compiler options, resulting in RTS library call...\n");
        printf("%.7f, %.7f,Error = %.7f\n",test_output,test_golden,(test_output-test_golden));
        if(fabsf(test_output-test_golden)<tolerance)
        {
            pass++;
    ...
    printf("Completed cosf test Pass_count = %d, Fail_count = %d \n", pass, fail);
    printf("cosf Cycles = %d, Timer overhead = %d \n", differenceCounter, overheadCounter);
}
